...
socket.on("data-server", function (msg) {
    ...
    function getPopupMoney(status, bet, result, money, fee) {
    let amount = parseFloat(Number(money));
    let finalAmount;

    if (status == 1) {
        switch (bet) {
            case "t":
                finalAmount = amount * 4.5;
                break;
            case "d":
                finalAmount = result == 0 ? amount * 1.5 : amount * 2;
                break;
            case "x":
                finalAmount = result == 5 ? amount * 1.5 : amount * 2;
                break;
            case "l":
            case "n":
                ...
                break;
            default:
                finalAmount = amount * 9;
                break;
        }
        return finalAmount.toFixed(2);
    } else if (status == 2) {
        finalAmount = amount + parseFloat(fee);
        return finalAmount.toFixed(2);
    }
    ...
    if (firstGame && firstGame.stage === list_orders[0].period) {
        ...
        if (firstGame.get == 0) {
            ...
            myModal_result.innerHTML = "LOSS :" + getPopupMoney(firstGame.status, firstGame.bet , firstGame.result , firstGame.money , firstGame.fee);
        } else {
            myModalheader.innerHTML = "Congratulations";
            myModal_result.innerHTML = "WIN :" + getPopupMoney(firstGame.status, firstGame.bet , firstGame.result , firstGame.money , firstGame.fee);
        }
...
// element_2.textContent = "4";
        fetch("/api/webapi/GetUserInfo")
            ...
            .then((data) => {
                ...
                $(".num span").text(`₹ ${parseFloat(data.data.money_user).toFixed(2)}`);
            });
        $(".Loading").fadeOut(0);
    }, 1000);
});
...
function alertBox(join, addText) {
    ...
    switch (join) {
        case "x":
            selectCss("colorgreen", "rgb(92, 186, 71)", "Green");
            break;
        case "t":
            selectCss("colorviolet", "rgb(152, 49, 233)", "Violet"+addText);
            break;
        case "d":
            selectCss("colorred", "rgb(251, 78, 78)", "Red"+addText);
            break;
        case "l":
            selectCss("colorbig", "rgb(255, 197, 17", "Big"+addText);
            break;
        case "n":
            selectCss("colorsmall", "rgb(92, 186, 71)", "Small"+addText);
            break;
        default:
            if (join % 2 == 0) {
...
$(".foot .right").click(function (e) {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            $(".total-box .num span").text("₹ " + parseFloat(response.money).toFixed(2));
                                ...
                                <div data-v-a9660e98="" class="money">
                                        ${
                                                ...
                                                ? '<span data-v-a9660e98="" class="success"> + ' +
                                                  parseFloat(Number(list_orders.money) * 4.5).toFixed(2) +
                                                  " </span>"

                                                :list_orders.status == 1 && list_orders.bet == "d" && list_orders.result == 0
                                                ? '<span data-v-a9660e98="" class="success"> + ' +
                                                parseFloat(Number(list_orders.money) * 1.5).toFixed(2) +
                                                " </span>"

                                                :list_orders.status == 1 && list_orders.bet == "x" && list_orders.result == 5
                                                ? '<span data-v-a9660e98="" class="success"> + ' +
                                                parseFloat(Number(list_orders.money) * 1.5).toFixed(2) +
                                                " </span>"

                                                : list_orders.status == 1 &&
                                                  list_orders.bet == "d" && list_orders.result != 0
                                                ? '<span data-v-a9660e98="" class="success"> + ' +
                                                parseFloat(Number(list_orders.money) * 2).toFixed(2) +
                                                  " </span>"
                                                : list_orders.status == 1 &&
                                                  list_orders.bet == "x"  && list_orders.result !=5
                                                ? '<span data-v-a9660e98="" class="success"> + ' +
                                                parseFloat(Number(list_orders.money) * 2).toFixed(2) +
                                                  " </span>"
                                                : list_orders.status == 1 &&
                                                ...
                                                : list_orders.status == 1
                                                ? '<span data-v-a9660e98="" class="success"> + ' +
                                                parseFloat(Number(list_orders.money) * 9).toFixed(2) +
                                                  " </span>"
                                                : list_orders.status == 2
                                                ? '<span data-v-a9660e98="" class="fail"> - ' +
                                                parseFloat(Number(list_orders.money)+Number(list_orders.fee)).toFixed(2) +
                                                  "</span>"
                                                : ""
                                        }
                                </div>
                                ...
                                <div data-v-a9660e98="" class="li c-row c-row-between c-row-middle">
                                    ...
                                    <div data-v-a9660e98="" class="li c-row c-row-between c-row-middle">
                                        ...
                                        <div data-v-a9660e98="" class="red">${
                                            parseFloat(list_orders.money).toFixed(2)
                                        }</div>
                                    </div>
                                    <div data-v-a9660e98="" class="li c-row c-row-between c-row-middle">
                                        <div data-v-a9660e98="">Tax</div>
                                        <div data-v-a9660e98="">${
                                            parseFloat(list_orders.fee).toFixed(2)
                                        }</div>
                                    </div>
                                    <div data-v-a9660e98="" class="li c-row c-row-between c-row-middle">
    ...
    $.ajax({
        ...
        success: function (response) {
            if (response.status === false) {
                ...
                $(
                    ".game-list .con-box:eq(0) .page-nav .van-icon-arrow-right"
                ).css("color", "#7f7f7f");
    ...
    $.ajax({
        ...
        success: function (response) {
            if (page - 1 <= 1) {
                ...
                $(
                    ".game-list .con-box:eq(0) .page-nav .van-icon-arrow-left"
                ).css("color", "#7f7f7f");
            }
            if (response.status === false) {
                pageno = 0;
    ...
    $.ajax({
        ...
        success: function (response) {
            if (response.status === false) {
                ...
                $(
                    ".game-list .con-box:eq(1) .page-nav .van-icon-arrow-right"
                ).css("color", "#7f7f7f");
                alertMessJoin(response.msg);
                return false;
            }
    ...
    $.ajax({
        ...
        success: function (response) {
            if (page - 1 <= 1) {
                ...
                $(
                    ".game-list .con-box:eq(1) .page-nav .van-icon-arrow-left"
                ).css("color", "#7f7f7f");
            }
            if (response.status === false) {
                pageno = 0;
...
).attr("src", "/img/wingo/clock_1.png");
$(
    `.game-betting .box .item:eq(${selectPageTime - 1}) `
    ) .css("background", "url(/img/wingo/time_1.svg) no-repeat center center ");
$(
    `.game-betting .box .item:eq(${selectPageTime - 1}) .triangle`
).css("border-top", ".24rem solid #FFC300");
$(
