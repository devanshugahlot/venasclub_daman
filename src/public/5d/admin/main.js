...
function messNewJoin2(datas) {
    ...
    datas.map((data) => {
        ...
        <div class="direct-chat-text" style="background-color: #1eb93d"> Tham gia ${
            isNumber(list_join2)
                ? "( " + list_join2 + " )"
                : list_join2 == "b"
                ? "Lớn"
...
function messNewJoin5(data) {
    ...
    for (let i = 0; i < bet.length; i++) {
        ...
        const totalMoney = money * Number(data.x);
        $(`#${bet[i]}`).attr("totalMoney", totalM + totalMoney);
        $(`#${bet[i]}`).text(totalM + totalMoney);

        let overallTotalMoney = Number($('#c').attr("totalMoney"));
        $('#c').attr("totalMoney", overallTotalMoney + totalMoney);
        $('#c').text(overallTotalMoney + totalMoney);
    }
}
